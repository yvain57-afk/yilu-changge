...
export const VOLLEY_GROUPS=5, VOLLEY_CAPACITY=47*VOLLEY_GROUPS;
...
export class Volleys {
 ...
 accept(events:readonly Feedback[]){for(const e of events){if(e.kind!=='shot'||!e.volley||e.projectileId===undefined||e.worldZ===undefined)continue;
  ...
  this.groups.push({id:e.projectileId,bornZ:e.worldZ,aimZ:e.volley.aimZ,height:e.volley.height,x:e.x,tick:e.simulationTick!,origins:formation(e.volley.count,e.x,e.volley.companion).filter((u,i)=>!u.hero&&!(e.volley!.companion&&i===1)).map(u=>{const m=bowMouth(u.hero);return{x:u.x+m.x,y:u.y+m.y-center.y};})});
 ...
 points(a:Arrow,z:number,bossDepth?:number){const g=this.groups.find(g=>g.id===a.id);if(!g)return[];
  const q=clamp((a.z-g.bornZ)/Math.max(.15,(bossDepth===undefined?g.aimZ:z+bossDepth)-g.bornZ),0,1),p=project(a.x,a.z-z),startScale=project(0,.35).s;
