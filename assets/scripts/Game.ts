...
type Screen='home'|'chapters'|'battle'|'pause'|'result'|'settings';
...
export class Game extends Component {
 ...
 async start(){view.setDesignResolutionSize(720,1280,ResolutionPolicy.FIXED_WIDTH);this.world=this.make('World',this.node);this.ui=this.make('Interface',this.node);this.battle=new BattleView(this.world);this.platform=new Platform(this.node,()=>{if(this.screen==='battle')this.pause();});this.text('loading','正在整队…',0,0,32);await this.platform.load();input.on(Input.EventType.TOUCH_START,this.touchStart);input.on(Input.EventType.TOUCH_MOVE,this.touchMove);input.on(Input.EventType.TOUCH_END,this.touchEnd);input.on(Input.EventType.TOUCH_CANCEL,this.touchCancel);profiler.hideStats();this.loaded=true;this.show('home');(globalThis as any).__YLCG__={snapshot:()=>this.snapshot()};}
 ...
 private show(screen:Screen){this.screen=screen;this.touchCancel();for(const n of [...this.ui.children]){n.removeFromParent();n.destroy();}this.hits=[];const l=LEVELS[this.level],b=this.platform.book,edge=Math.max(0,(view.getVisibleSize().height-1280)/2);
  if(screen!=='battle'&&screen!=='pause'&&screen!=='settings')this.battle.render(null,b.rank!=='布衣');
  if(screen==='home'){
   this.panel(0,330,656,410);this.text('eyebrow','起步篇 / 第一关草模',0,478,23,MUTE);this.text('title','一路长歌',0,385,67);this.text('tagline','从小人物起步，一步步当皇帝',0,302,27);this.text('scope','计划三关：拿下第一座城\n本次开放第一关 · 临时造型',0,218,23,MUTE,600,80);
   this.text('rank',`布衣 → 头领 → 统领 → 城主`,0,-120,25);this.button('start',b.data.cleared[l.id]?'再战第一关':'开始闯关',-225,()=>this.begin(),0,530,true);this.button('chapters','关卡与晋升',-325,()=>this.show('chapters'));this.button('settings','设置',-425,()=>{this.returnScreen='home';this.show('settings')});this.text('help','左右拖动，自动射箭 · 兵力越多，火力越强',0,-523,21);
  ...
  this.text('prototype','S1 草模 · 临时人物 / 非正式美术',0,627+edge,17,MUTE,650,30);if(b.notice)this.text('notice',b.notice,0,-626-edge,18,RED,690,30);
 }
 private begin(){this.journey?.dispose();this.journey=new Journey(LEVELS[this.level]);this.message=`本次出征：${this.journey.count}人`;this.messageTime=3;this.tutorialUntil=5;this.dragged=false;this.frameTimes=[];this.show('battle');this.platform.syncAudio();}
 private leave(){this.journey?.dispose();this.platform.syncAudio(false);this.show('chapters');}
 ...
 update(dt:number){if(!this.loaded)return;this.wallFrames++;this.fpsElapsed+=dt;if(this.fpsElapsed>=1){this.fps=this.wallFrames/this.fpsElapsed;this.wallFrames=0;this.fpsElapsed=0;}
  if(this.screen==='battle'&&this.journey){const j=this.journey;this.frameTimes.push(dt);if(this.frameTimes.length>7200)this.frameTimes.shift();j.advance(dt);let hit=false;for(const e of j.drainFeedback()){if(e.kind==='hit'){hit=true;continue;}this.platform.sound(e.kind);if(e.kind==='gather'){this.message=`增兵 +${e.amount}`;this.messageTime=1.5;}if(e.kind==='hurt'){this.message=`损失 ${e.amount} 人`;this.messageTime=2;this.platform.vibrate();}}
