...
export class Game extends Component {
 ...
 private firstVictory=false;private finishAge=0;private promotionTime=0;private promotionView:BattleView|null=null;
 ...
 private show(screen:Screen){this.screen=screen;this.promotionView=null;this.promotionTime=0;this.touchCancel();for(const n of [...this.ui.children]){n.removeFromParent();n.destroy();}this.hits=[];const l=LEVELS[this.level],b=this.platform.book,edge=Math.max(0,(view.getVisibleSize().height-1280)/2);
  if(screen!=='battle'&&screen!=='pause'&&screen!=='settings'&&screen!=='preview')this.battle.render(null,rankStage(b));
  if(screen==='home'){
   this.panel(0,352,620,350);this.text('eyebrow','起步篇 · 三关，拿下第一座城。',0,478,23,MUTE);this.text('title','一路长歌',0,390,72);this.text('tagline','从小人物起步，一步步当皇帝。',0,302,27);this.text('scope','山道聚义 → 拿下营寨 → 夺下首城',0,227,23,MUTE,600,80);
   this.text('rank',`布衣 → 头领 → 统领 → 城主`,0,-120,25);this.button('start',nextUnfinished(b)<0?'再次挑战':rankStage(b)>0?'继续闯关':'开始闯关',-225,()=>{const i=nextUnfinished(b);if(i<0)this.show('chapters');else this.begin(i);},0,530,true);this.button('chapters','关卡与晋升',-325,()=>this.show('chapters'));this.button('settings','设置',-425,()=>{this.returnScreen='home';this.show('settings')});this.text('current-rank',`当前身份 · ${b.rank}`,0,-525,24,MUTE);
   ...
   const title=!won?'胜败乃兵家常事':!this.firstVictory?'再次取胜':last?'首城已得':`晋升${l.rankAfter}`;
   this.text('title',title,0,470,43);this.text('ending',won?(last?'起步篇完成 · 登基之路，刚刚开始':l.ending):this.level===2&&j.z===j.level.duration?j.cause.replace('弩箭','羽箭'):j.cause,0,364,25,INK,560,100);
   this.text('identity',won?(this.firstVictory?`${l.rankBefore} → ${l.rankAfter} · 进度已保存`:`当前身份：${b.rank} · 战绩已保存`):`当前身份：${b.rank} · 本关免费重开`,0,277,25);
   if(won){const portrait=this.make('promotion-hero',this.ui);portrait.setPosition(0,-5);this.promotionView=new BattleView(portrait);this.promotionView.portrait(rankStage(b),0,this.level);}
   this.text('stats',`剩余兵力：${j.count}  /  最佳：${b.data.best[l.id]} 人\n用时：${j.elapsed.toFixed(1)} 秒`,0,-133,24,MUTE,570,95);
   if(won&&!last)this.button('next','下一关：'+LEVELS[this.level+1].title,-260,()=>this.begin(this.level+1),0,530,true);
   ...
   this.panel(0,0,640,820);this.text('title','设置',0,300,44);(['music','sfx','vibration'] as const).forEach((k,i)=>this.button(k,`${['背景音乐','动作音效','轻触震动'][i]}：${b.data.settings[k]?'开':'关'}`,140-i*110,()=>{b.data.settings[k]=!b.data.settings[k];b.persist();this.platform.syncAudio(this.returnScreen!=='pause');this.show('settings')}));this.text('music-credit','音乐来源：Shenyang — Kevin MacLeod\n(incompetech.com) · CC BY 4.0\ncreativecommons.org/licenses/by/4.0/\n完整曲目转码 · 音量调整',0,-212,18,MUTE,580,120);this.button('return','返回',-320,()=>this.show(this.returnScreen));
  }
  this.text('prototype','一路长歌 · v0.3.1 三关试玩',0,627+edge,17,MUTE,650,30);if(b.notice)this.text('notice',b.notice,0,-626-edge,18,RED,690,30);
 }
 private begin(index=this.level){if(!canStart(this.platform.book,index,ArtSprites.ready)){this.message='本关尚未解锁或资源未就绪';return;}this.level=index;this.firstVictory=false;this.finishAge=0;this.platform.stopEffects();this.battle.clear();this.journey?.dispose();this.journey=new Journey(LEVELS[this.level]);this.message=`本次出征：${this.journey.count}人`;this.messageTime=3;this.tutorialUntil=5;this.dragged=false;this.frameTimes=[];this.show('battle');this.platform.syncAudio();}
 ...
 update(dt:number){if(!this.loaded)return;this.wallFrames++;this.fpsElapsed+=dt;if(this.fpsElapsed>=1){this.fps=this.wallFrames/this.fpsElapsed;this.wallFrames=0;this.fpsElapsed=0;}
  if(this.screen==='result'&&this.promotionView&&!this.platform.hidden){this.promotionTime+=dt;this.promotionView.portrait(rankStage(this.platform.book),this.firstVictory?this.promotionTime:2,this.level);}
