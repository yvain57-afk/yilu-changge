...
export class Game extends Component {
 ...
 private finishAge=0;private promotionTime=0;private promotionView:BattleView|null=null;
 ...
 private show(screen:Screen){this.screen=screen;this.promotionView=null;this.promotionTime=0;this.touchCancel();for(const n of [...this.ui.children]){n.removeFromParent();n.destroy();}this.hits=[];const l=LEVELS[this.level],b=this.platform.book,edge=Math.max(0,(view.getVisibleSize().height-1280)/2);
  ...
  if(screen==='home'){
   this.panel(0,352,620,350);this.text('eyebrow','起步篇 · 山道聚义',0,478,23,MUTE);this.text('title','一路长歌',0,390,72);this.text('tagline','从小人物起步，一步步当皇帝',0,302,27);this.text('scope','第一关 · 拉起队伍\n仅开放本关，通关晋升头领',0,227,23,MUTE,600,80);
   this.text('rank',`布衣 → 头领 → 统领 → 城主`,0,-120,25);this.button('start',b.data.cleared[l.id]?'再战第一关':'开始闯关',-225,()=>this.begin(),0,530,true);this.button('chapters','关卡与晋升',-325,()=>this.show('chapters'));this.button('settings','设置',-425,()=>{this.returnScreen='home';this.show('settings')});this.button('preview','查看队伍与动作',-525,()=>this.show('preview'));
   ...
   const j=this.journey!,won=j.phase==='won';this.panel(0,30,648,1120);this.text('title',won?'聚众成军 · 晋升头领':'胜败乃兵家常事',0,470,40);this.text('ending',won?l.ending:j.cause,0,360,25,INK,560,100);this.text('identity',won?'布衣 → 头领 · 晋升已保存':'重新整队，再来一次',0,270,26);if(won){const portrait=this.make('promotion-hero',this.ui);portrait.setPosition(0,-5);this.promotionView=new BattleView(portrait);this.promotionView.portrait(false,0);}this.text('stats',`剩余兵力：${j.count}  /  最佳：${b.data.best[l.id]}\n用时：${j.elapsed.toFixed(1)} 秒`,0,-108,24,MUTE,570,95);this.text('stop','第一关已开放 · 下一段长歌，待续',0,-210,22,MUTE,560,60);this.button('restart',won?'再战一次':'重新开始',-325,()=>this.begin(),0,530,true);this.button('chapters','返回关卡',-425,()=>this.leave());
  ...
  this.text('prototype','一路长歌 · S2-B 试制',0,627+edge,17,MUTE,650,30);if(b.notice)this.text('notice',b.notice,0,-626-edge,18,RED,690,30);
 }
 private begin(){this.finishAge=0;this.platform.stopEffects();this.battle.clear();this.journey?.dispose();this.journey=new Journey(LEVELS[this.level]);this.message=`本次出征：${this.journey.count}人`;this.messageTime=3;this.tutorialUntil=5;this.dragged=false;this.frameTimes=[];this.show('battle');this.platform.syncAudio();}
 ...
 update(dt:number){if(!this.loaded)return;this.wallFrames++;this.fpsElapsed+=dt;if(this.fpsElapsed>=1){this.fps=this.wallFrames/this.fpsElapsed;this.wallFrames=0;this.fpsElapsed=0;}
  if(this.screen==='result'&&this.promotionView&&!this.platform.hidden){this.promotionTime+=dt;this.promotionView.portrait(false,this.promotionTime);}
