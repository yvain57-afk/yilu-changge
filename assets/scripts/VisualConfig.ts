...
export function formation(count:number,x:number){return Array.from({length:Math.min(48,Math.max(0,count))},(_,i)=>{if(i===0)return{x:x*260,y:VISUAL.teamY,hero:true};const row=Math.floor((i-1)/FORMATION_COLUMNS),col=(i-1)%FORMATION_COLUMNS,cols=Math.min(FORMATION_COLUMNS,count-1-row*FORMATION_COLUMNS);const half=(cols-1)*24,center=clamp(x*260+(row%2?12:-12),-310+half,310-half);return{x:center+(col-(cols-1)/2)*48,y:VISUAL.teamY-58-row*31,hero:false};});}
