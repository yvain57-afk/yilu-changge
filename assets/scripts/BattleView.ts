...
export class BattleView {
 ...
 render(j:Journey|null,rank:RankStage=0,finishAge=0){this.begin();this.currentRank=rank;this.currentScene=j?Math.max(0,Number(j.level.id.slice(-2))-1):rank===3?2:rank===2?1:0;this.ground(j?.z||0,this.currentScene,j?j.level.duration:6,j?.phase==='won'||!j&&rank>0);if(!j){this.use('hero-home');this.sprite('home',('portrait'+rank) as Art,0,-65,200/330);return;}
  ...
  if(j.phase==='boss'||j.phase==='won'||j.z>j.level.duration-4){const d=BOSS_TARGET.depth+(j.phase==='run'?j.level.duration-j.z:0),p=project(0,d),width=bossFootprint(d),w=j.bossWarning;const frame=j.phase==='won'?3:w?.stage==='charge'?1:w?.stage==='flight'?2:w?.stage==='impact'||j.bossClock<.5&&j.bossIndex>0?3:0;const hit=this.effects.items.some(f=>f.event.kind==='hit'&&f.event.targetId==='boss'&&j.elapsed-f.born<.1);jobs.push({id:'boss',y:p.y,draw:()=>{
   // Per-boss body/equipment alpha measurements: evidence/v03/boss-silhouette.json.
