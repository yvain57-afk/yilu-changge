...
export const ART_FRAMES={...S2B_FRAMES,...V03_FRAMES};
...
export class ArtSprites {
 ...
 static async load(){await Promise.all(['actors','scenery','v4/enemies','v4/heroes','v4/scenery'].map(sheet=>new Promise<void>((resolve,reject)=>resources.load(`${sheet.startsWith('v4/')?sheet:'v3/'+sheet}/texture`,Texture2D,(e,t)=>{if(e){reject(e);return;}for(const name of Object.keys(ART_FRAMES) as Art[]){const d=ART_FRAMES[name];if(d.sheet!==sheet)continue;const [x,y,w,h]=d.rect,f=new SpriteFrame();f.texture=t;f.rect=new Rect(x,y,w,h);f.originalSize=new Size(w,h);f.offset=new Vec2();ArtSprites.frames[name]=f;}resolve();}))));}
