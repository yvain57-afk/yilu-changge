...
try{
 ...
 report={passed:true,background,returnedPaused:true,explicitContinueRestoresProgress:true,browser:version.Browser};const shot=await cmd('Page.captureScreenshot',{format:'png'});writeFileSync('evidence/v02-s2/lifecycle-native.png',Buffer.from(shot.data,'base64'));console.log(JSON.stringify(report));
}catch(e){writeFileSync('evidence/v02-s2/lifecycle-events-failure.json',JSON.stringify(events.filter(m=>['Runtime.exceptionThrown','Page.loadEventFired','Page.lifecycleEvent'].includes(m.method)),null,2));report={...report,error:String(e)};console.error(e);process.exitCode=1;}finally{writeFileSync('evidence/v02-s2/browser-lifecycle.json',JSON.stringify(report,null,2));if(ws?.readyState===1){await send('Browser.close');ws.close();}chrome.kill('SIGTERM');}
