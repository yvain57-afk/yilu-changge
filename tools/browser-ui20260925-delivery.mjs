import {open,steer,freshSave,root,startCapture,stopCapture} from './browser-ui20260925-common.mjs';
import assert from 'node:assert/strict';
import {writeFileSync,mkdirSync} from 'node:fs';
const attemptId=new Date().toISOString().replace(/[:.]/g,'-'),attemptDir=root+'/runs/'+attemptId;mkdirSync(attemptDir,{recursive:true});
const t=await open('natural-delivery',390,844,{audio:true}),runs=[],timeline=[],report={attemptId,attemptDir,passed:false,scope:'One real-time continuous three-level run driven only by touch input; no injected troops, HP, equipment or time'},start=Date.now();
...
try{
 ...
 for(let i=0;i<3;i++){
  ...
  while((await t.snap()).screen==='battle'){
   ...
   assert.equal(j.awakeningUsed,false);assert.equal(j.melee,null);assert.equal(j.waves.length,0);assert.ok(r.stats.peakShots<=512&&r.stats.peakEnemies<=64);assert.ok(Date.now()-start<360000,'run bounded to six minutes');await t.p.waitForTimeout(20);
  }
  ...
  mark('victory',i+1);runs.push({level:i+1,seconds:j.elapsed,count:j.count,stage:r.stage,remaining:r.remaining,bossHP:j.bossHP,distanceLeft:r.distanceLeft,stats:r.stats,ledger:r.ledger,lastDamage:r.lastDamage,meanFPS:end.meanFPS,frameTiming:end.frameTiming,pacing:r.pacing,samples});console.log(JSON.stringify({event:'level-complete',level:i+1,seconds:j.elapsed,count:j.count,stage:r.stage}));await t.p.waitForTimeout(300);await t.tap('transition');await t.p.waitForTimeout(250);
  if(i<2){await t.tap(i===0?'weapon-blade':'weapon-spear');await t.tap(i===0?'companion-xing_daorong':'companion-chen_ying');}
  mark('transition',i+1);const flagState=await t.snap(),src=flagState.presentation.draggableFlag,dst=flagState.presentation.dropTarget;assert.ok(src&&dst);const a=await t.pos(src.x,src.y),b=await t.pos(dst.x+src.offsetX,dst.y+src.offsetY);await t.cd.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:a.x,y:a.y,id:7}]});for(let q=1;q<=10;q++){await t.cd.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:a.x+(b.x-a.x)*q/10,y:a.y+(b.y-a.y)*q/10,id:7}]});await t.p.waitForTimeout(40);}await t.cd.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});assert.equal((await t.snap()).transition.done,true);await t.p.waitForTimeout(1400);
 }
 assert.equal((await t.snap()).screen,'meeting');await t.p.waitForTimeout(1500);while((await t.snap()).screen==='meeting'){await t.tap('return-camp');await t.p.waitForTimeout(400);}await t.p.waitForTimeout(600);mark('ending',3);report.sound=await stopCapture(t,root+'/final-three-levels.webm');assert.ok(report.sound.audibleSamples>10);
...
}catch(e){report.error=String(e);try{report.failedState=await t.snap();}catch{}console.error(e);process.exitCode=1;try{if(!report.sound)report.sound=await stopCapture(t,attemptDir+'/incomplete-natural-run.webm');}catch{}}
finally{Object.assign(report,{runs,timeline,errors:t.errors});writeFileSync(root+'/natural-flow.json',JSON.stringify(report,null,2));writeFileSync(attemptDir+'/report.json',JSON.stringify(report,null,2));await t.c.close();console.log(JSON.stringify({passed:report.passed,error:report.error,runs:runs.map(r=>({level:r.level,seconds:r.seconds,count:r.count}))}));}
