...
const files=[...walk('assets/scripts'),...walk('assets/resources'),'package.json','package-lock.json','settings/v2/packages/builder.json'].filter(p=>!p.endsWith('.meta')).sort();
...
writeFileSync('evidence/v05/fingerprint.json',JSON.stringify({version:'v05-rc1',sourceHash,source,web:bundle('build/web-mobile'),wechat:{status:'not rebuilt or uploaded for v05'},localTestIdentityExcluded:true},null,2));console.log(sourceHash);
