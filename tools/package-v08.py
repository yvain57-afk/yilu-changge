"""Create the complete, account-neutral v08 handoff without editor caches."""
from pathlib import Path
import argparse, datetime, hashlib, json, shutil, zipfile

ROOT = Path(__file__).resolve().parents[1]
DEST = ROOT / 'release/v08'
STAGE = DEST / 'YILU_V08_FULL_DEV'
DIRS = ['assets', 'settings', 'art-source', 'music-source', 'references', 'tests', 'tools', 'docs', 'build/web-mobile', 'build/wechatgame', 'evidence/v08', 'evidence/runner-video-v2']
...
def prepare():
    ...
    if not handoff.exists():
        handoff = Path((ROOT / '.cache/v08-handoff-path.txt').read_text())
    shutil.copy2(handoff, STAGE / 'AGENT_HANDOFF.md')
    (STAGE / '00_START_HERE.md').write_text('''# 一路长歌 v0.8 完整开发包

先读 [给 GPT 的反馈](docs/v08/GPT_REVIEW.md)，再读 [运行与开发说明](docs/v08/DEVELOPMENT_PACKAGE.md)。
...
- `docs/v08/GPT_REVIEW.md`：下一轮具体问题、证据、代码入口和用户反馈表。
- `docs/v08/source-pack/YILU_LASTWAR_VIDEO_ALIGNED_V2/02_EVIDENCE.md`：视频机制与时间点，区别观察和推测。
- `evidence/v08/three-levels-live-audio.mp4`：原速连续三关。
- `evidence/v08/mobile-short-web.png`：最终小屏画面。
- `evidence/runner-video-v2/summary.json`、`phone-preview.json`：当前验证状态。
...
def archive():
    ...
    manifest = {'version': '0.8.0-rc1', 'generatedAt': datetime.datetime.now().astimezone().isoformat(), 'snapshot': 'current working tree including uncommitted work', 'includes': DIRS + FILES, 'excludes': ['Git history', 'dependencies and editor caches', 'personal editor profiles', 'login credentials', 'temporary preview QR', 'duplicate historical ZIP/WebM', 'full third-party video'], 'sanitized': {'build/wechatgame/project.config.json': 'appid blanked in shared package; local build unchanged'}, 'fileCount': len(entries), 'totalBytes': sum(x['bytes'] for x in entries), 'files': entries}
    (STAGE / 'MANIFEST.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    out = DEST / 'YILU_V08_FULL_DEV.zip'
