"""Prepare, then archive a verified account-neutral BATTLE-REWORK-20260925 local working-tree snapshot."""
from pathlib import Path
import argparse, datetime, hashlib, json, re, shutil, zipfile

ROOT = Path(__file__).resolve().parents[1]
DEST = ROOT / 'release/BATTLE-REWORK-20260925'
STAGE = DEST / 'YILU_BATTLE_REWORK_20260925_FULL_DEV'
DIRS = ['assets', 'settings', 'art-source', 'music-source', 'references', 'tests', 'tools', 'docs', 'build/web-mobile', 'build/wechatgame', 'evidence/BATTLE-REWORK-20260925', 'evidence/UI-20260925', 'evidence/v09', 'evidence/v081', 'evidence/v08', 'evidence/runner-video-v2']
...
def allowed(p):
    name = p.name.lower()
    return not any(x in p.parts for x in ['node_modules', '.cache', '__pycache__', '.git', 'downloads']) and name not in ['.ds_store', 'project.private.config.json'] and not any(s in name for s in ['preview-qr', 'preview-result', '二维码', 'login', 'credential']) and p.suffix.lower() not in ['.zip', '.pem', '.key'] and not name.startswith('.env')
...
def prepare():
    ...
    if json.loads((ROOT / 'package.json').read_text())['version'] != '0.9.2-battle20260925':
        raise SystemExit('Unexpected project version; review snapshot before packaging.')
    for path in ['docs/BATTLE-REWORK-20260925/00_START_WITH_CODEX.md','docs/BATTLE-REWORK-20260925/REVIEW_RESULTS.json','evidence/BATTLE-REWORK-20260925/VERIFICATION.json','evidence/BATTLE-REWORK-20260925/wechat-package.json','evidence/BATTLE-REWORK-20260925/final-three-levels.mp4']:
    ...
    shutil.copy2(ROOT / 'docs/BATTLE-REWORK-20260925/00_START_WITH_CODEX.md', STAGE / '00_START_HERE.md')
...
def archive():
    ...
    manifest = {'version': '0.9.2-battle20260925', 'runtime': 'battle-rework-20260925', 'generatedAt': datetime.datetime.now().astimezone().isoformat(), 'snapshot': 'current local working tree including uncommitted work; not a GitHub HEAD claim', 'includes': DIRS + FILES, 'excludes': ['Git history', 'dependencies and editor caches', 'personal editor profiles', 'login credentials', 'preview QR', 'duplicate ZIP', 'full third-party video'], 'sanitized': {'build/wechatgame/project.config.json': 'AppID blanked in shared stage only; local build unchanged'}, 'fileCount': len(entries), 'totalBytes': sum(x['bytes'] for x in entries), 'files': entries}
    (STAGE / 'MANIFEST.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + '\n')
    out = DEST / 'YILU_BATTLE_REWORK_20260925_FULL_DEV.zip'
    if out.exists():
