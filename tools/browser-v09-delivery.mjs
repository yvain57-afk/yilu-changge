import {open,steer,freshSave,root,startCapture,stopCapture} from './browser-v09-common.mjs';
...
try{
 ...
 assert.equal((await t.snap()).screen,'meeting');await t.p.waitForTimeout(1500);while((await t.snap()).screen==='meeting'){await t.tap('return-camp');await t.p.waitForTimeout(400);}await t.p.waitForTimeout(600);mark('ending',3);report.sound=await stopCapture(t,root+'/three-levels-live-audio.webm');assert.ok(report.sound.audibleSamples>10);
...
}catch(e){report.error=String(e);try{report.failedState=await t.snap();}catch{}console.error(e);process.exitCode=1;try{if(!report.sound)report.sound=await stopCapture(t,root+'/incomplete-natural-run.webm');}catch{}}
