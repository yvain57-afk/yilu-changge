...
const chrome=spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',['--remote-debugging-port=43188',`--user-data-dir=${resolve('.cache/browser-lifecycle')}`,'--no-first-run','--no-default-browser-check','about:blank'],{stdio:'ignore'});
...
try{
 ...
 await tap('chapters');await tap('level0');await tap('start');await delay(400);assert.equal((await snapshot()).screen,'battle');
 ...
 report={passed:true,background,returnedPaused:true,explicitContinueRestoresProgress:true,browser:version.Browser};const shot=await cmd('Page.captureScreenshot',{format:'png'});writeFileSync('evidence/lifecycle-native.png',Buffer.from(shot.data,'base64'));console.log(JSON.stringify(report));
}catch(e){writeFileSync('evidence/lifecycle-events-failure.json',JSON.stringify(events.filter(m=>['Runtime.exceptionThrown','Page.loadEventFired','Page.lifecycleEvent'].includes(m.method)),null,2));report={...report,error:String(e)};console.error(e);process.exitCode=1;}finally{writeFileSync('evidence/browser-lifecycle.json',JSON.stringify(report,null,2));if(ws?.readyState===1){await send('Browser.close');ws.close();}chrome.kill('SIGTERM');}
